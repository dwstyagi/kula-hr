module UiHelper
  # Sidebar navigation link. Active state matches on path prefix unless
  # `exact: true` (used for portal roots so they don't match every subpath).
  def nav_link(label, path, icon:, exact: false, count: nil)
    active = exact ? current_page?(path) : request.path.start_with?(path)

    link_to path, class: "nav-item #{'nav-item-active' if active}" do
      safe_join([
        nav_icon(icon),
        content_tag(:span, label, class: "flex-1 truncate"),
        (count_pill(count) if count&.positive?)
      ].compact)
    end
  end
  ...
  STATUS_BADGES = {
    # success
    "approved" => "badge-success", "paid" => "badge-success", "active" => "badge-success",
    "published" => "badge-success", "completed" => "badge-success", "submitted" => "badge-success",
    "present" => "badge-success", "verified" => "badge-success",
    # warning / pending attention
    "pending" => "badge-warning", "under_review" => "badge-warning", "trial" => "badge-warning",
    "on_probation" => "badge-warning", "probation" => "badge-warning", "half_day" => "badge-warning",
    # danger
    "rejected" => "badge-danger", "cancelled" => "badge-danger", "failed" => "badge-danger",
    "suspended" => "badge-danger", "inactive" => "badge-danger", "terminated" => "badge-danger",
    "absent" => "badge-danger", "overdue" => "badge-danger",
    # processing / in-flight
    "processing" => "badge-processing", "processed" => "badge-info", "in_progress" => "badge-processing",
    # neutral
    "draft" => "badge-neutral", "archived" => "badge-neutral", "on_leave" => "badge-info",
    "resigned" => "badge-neutral", "notice_period" => "badge-warning"
  }.freeze

  def status_badge(status, label: nil)
    key = status.to_s
    content_tag :span, (label || key.humanize),
      class: "badge #{STATUS_BADGES.fetch(key, 'badge-neutral')}"
  end

  # Rupee amount with tabular numerals.
  ...
  # Line icons (24px viewBox, 1.5 stroke) used by the sidebars.
  NAV_ICONS = {
    ...
    map_pin: "M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm4.5 0c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z",
    ...
    identification: "M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm4.125-9.75a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm1.294 6.336a6.721 6.721 0 01-3.17.789 6.721 6.721 0 01-3.168-.789 3.376 3.376 0 016.338 0z",
    ...
    logout: "M15.75 9V5.25A2.25 2.25 0 0013.5 3h-7.5A2.25 2.25 0 003.75 5.25v13.5A2.25 2.25 0 006 21h7.5a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9",
  ...
  def nav_icon(name)
    path = NAV_ICONS.fetch(name.to_sym)
    content_tag :svg, tag.path(d: path, "stroke-linecap": "round", "stroke-linejoin": "round"),
      class: "size-5", fill: "none", stroke: "currentColor", "stroke-width": "1.5", viewBox: "0 0 24 24", "aria-hidden": "true"
  end

  # Initials avatar chip used in topbars and sidebars.
  def avatar_chip(name_or_initials, dark: false)
    initials = name_or_initials.to_s.split.map { |w| w[0] }.join[0, 2].upcase
    content_tag :div, initials,
      class: "size-8 shrink-0 rounded-full flex items-center justify-center text-xs font-bold " \
             "#{dark ? 'bg-white/10 text-kula-100' : 'bg-kula-100 text-kula-800'}"
  end
end
